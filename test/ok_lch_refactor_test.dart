...
void main() {
  group('OkLchColor Refactor Tests', () {
    test('whiten increases lightness', () {
      final OkLCH color = OkLCH.alt(const Percent(0.5), 0.2, 0); // Red-ish
    ...
    test('lerp interpolates correctly', () {
      final OkLCH start = OkLCH.alt(const Percent(0.5), 0.2, 0); // Red-ish
      final OkLCH end = OkLCH.alt(const Percent(0.5), 0.2, 120); // Green-ish
    ...
    test('lerp handles hue wrapping', () {
      final OkLCH start = OkLCH.alt(const Percent(0.5), 0.2, 350);
      final OkLCH end = OkLCH.alt(const Percent(0.5), 0.2, 10);
      final OkLCH mid = start.lerp(end, 0.5);
    ...
    test('adjustHue works', () {
      final OkLCH color = OkLCH.alt(const Percent(0.5), 0.2, 0);
    ...
    test('cooler shifts hue towards 210', () {
      final OkLCH color = OkLCH.alt(const Percent(0.5), 0.2, 180);
