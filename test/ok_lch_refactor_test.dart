...
void main() {
  group('OkLchColor Refactor Tests', () {
    ...
    test('lerp interpolates correctly', () {
      const OkLCH start = OkLCH(Percent(0.5), 0.2, 0); // Red-ish
      const OkLCH end = OkLCH(Percent(0.5), 0.2, 120); // Green-ish
    ...
    test('lerp handles hue wrapping', () {
      const OkLCH start = OkLCH(Percent.mid, 0.2, 350);
      const OkLCH end = OkLCH(Percent.mid, 0.2, 10);
      final OkLCH mid = start.lerp(end, 0.5);
    ...
    test('adjustHue works', () {
      const OkLCH color = OkLCH(Percent.mid, 0.2, 0);
    ...
    test('cooler shifts hue towards 210', () {
      const OkLCH color = OkLCH(Percent.mid, 0.2, 180);
