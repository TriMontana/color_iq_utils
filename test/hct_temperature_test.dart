...
void main() {
  group('HCT and Temperature Tests', () {
    ...
    test('Transparency Adjustment', () {
      ...
      final ColorIQ transparent = color.adjustTransparency(50); // Reduce opacity by 50%
      expect(transparent.transparency, closeTo(0.5, 0.01));
      expect(transparent.alpha, closeTo(127, 2));

      final ColorIQ moreTransparent = color.adjustTransparency(80); // Reduce by 80% -> 20% opacity
    ...
    test('Color Temperature', () {
      // Warm colors
      expect(const ColorIQ.fromARGB(255, 255, 0, 0).temperature, ColorTemperature.warm); // Red
      expect(const ColorIQ.fromARGB(255, 255, 165, 0).temperature, ColorTemperature.warm); // Orange
      expect(const ColorIQ.fromARGB(255, 255, 255, 0).temperature, ColorTemperature.warm); // Yellow

      // Cool colors
      expect(const ColorIQ.fromARGB(255, 0, 255, 0).temperature, ColorTemperature.cool); // Green
      expect(const ColorIQ.fromARGB(255, 0, 0, 255).temperature, ColorTemperature.cool); // Blue
      expect(const ColorIQ.fromARGB(255, 0, 255, 255).temperature, ColorTemperature.cool); // Cyan
      ...
      expect(const ColorIQ.fromARGB(255, 128, 0, 128).temperature, ColorTemperature.warm); 

      // Blue (240) -> Cool.
      expect(const ColorIQ.fromARGB(255, 0, 0, 255).temperature, ColorTemperature.cool);
    });
    ...
    test('HctColor Temperature', () {
      ...
      const HctColor redHct = HctColor(27, 100, 50);
      expect(redHct.temperature, ColorTemperature.warm);

      const HctColor blueHct = HctColor(260, 100, 50);
