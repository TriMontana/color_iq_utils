...
class Color extends ColorIQ {
  ...
  factory Color.fromRgb(final int r, final int g, final int b) => Color.fromARGB(255, r, g, b);

  /// Parse a hex string such as `#FF00FF` or `FF00FF` (ARGB or RGB).
  factory Color.fromHex(final String hex) {
    final String normalized = hex.trim().startsWith('#') ? hex.trim() : '#${hex.trim()}';
  ...
  factory Color.fromCss(final String css) => Color(CssColor.fromCssString(css).toColor().value);
